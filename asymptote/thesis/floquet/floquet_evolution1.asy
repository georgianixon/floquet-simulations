...
real y_hfline = func_a1*func_a2-0.9;
real y_uline = y_hfline+2.4;
real y_timeline = -func_a1*func_a2;

// for the function and t line
real x_func_0 = -0.2*T1;
real x_func_f = tf + 0.1*T1;

// for label gaps
real y_label_gap = 0.8;
real x_timelabel_gap = 0.05*T1;
real y_linespace = 0.1*T1;
pen pen_lab_fontsize = fontsize(8pt);
...
draw(func_path, p=linewidth(1pt),arrow=ArcArrow(SimpleHead, size=4));


// draw hf horizontal lines
draw((t0 + x_horizontal_line_gap, y_hfline) -- (t0 + T1 - x_horizontal_line_gap,y_hfline), p=rgb(colour3)+linewidth(1pt), arrow=ArcArrow(SimpleHead, size=4));
draw((t0 + T1 +x_horizontal_line_gap, y_hfline) -- (t0 + 2*T1 - x_horizontal_line_gap,y_hfline), p=rgb(colour3)+linewidth(1pt), arrow=ArcArrow(SimpleHead, size=4));
...
// hf labels
// label("$U(t_0+T, t_0)$ ",(t0 + T1/2, y_hfline + y_label_gap), p=rgb(colour3));
label("$\mathrm{e}^{-iTH_S^{t_0}/\hbar}$",(t0 + T1/2, y_hfline + y_label_gap), p=rgb(colour3)+pen_lab_fontsize);
label("$\mathrm{e}^{-iTH_S^{t_0}/\hbar}$",(t0 + 1.5*T1, y_hfline + y_label_gap), p=rgb(colour3)+pen_lab_fontsize);
// label("$U(t_{\mathrm{f}}, t_0+T)$ ",((t0 + T1)/2 + tf/2, y_hfline + y_label_gap), p=rgb(colour1));
label("$P(t_{\mathrm{f}}, t_0) \times$",(t0/2 + T1 + tf/2, y_hfline + y_label_gap), p=rgb(colour1)+pen_lab_fontsize);
label("$\mathrm{e}^{-i(t_{\mathrm{f}} - t_0 - 2T)H_S^{t_0}/\hbar}$",((t0 + 2*T1)/2 + tf/2, y_hfline - y_label_gap), p=rgb(colour1)+pen_lab_fontsize);
...
// u label
label("$P(t_{\mathrm{f}}, t_0) \mathrm{e}^{-i(t_{\mathrm{f}} - t_0)H_S^{t_0}/\hbar}$",(t0/2 + tf/2, y_uline + y_label_gap+0.2), p=rgb(colour2)+pen_lab_fontsize);

// grey lines
draw ((t0, y_uline) -- (t0, y_timeline), p=linetype("2 2")+linewidth(0.7pt)+rgb("6C6C6C"));
draw ((t0 + T1, y_hfline) -- (t0+T1, y_timeline), p=linetype("2 2")+linewidth(0.7pt)+rgb("6C6C6C"));
...
label("$t$", (x_func_f + x_timelabel_gap, y_timeline));
...
label("$H(t)$", (x_func_f,func_t(x_func_f))+(y_label_gap,y_label_gap));
