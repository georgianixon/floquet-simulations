...
string colour_u = colour1;
string colour_e = colour3;
...
label("$x$",axes_start+ (2*pi/u_w*0.5, 0) );

// a size double arrow
real lattice_const_arrow_y = u_amp*exp_amp*-0.06;
...
label("$a$", (2*pi/u_w*0.25, lattice_const_arrow_y*2));

// ux label
pair ux_label_pos = (2*pi/u_w*2.1,u_amp*exp_amp*0.1);
label("$e^{i k x} u_{k}(x))$", (ux_label_pos),  p=rgb(colour_u));

// exp label
pair exp_label_pos = (2*pi/u_w*1.3,u_amp*exp_amp*0.8);
label("$e^{i k x}$", (exp_label_pos),  p=rgb(colour_e));